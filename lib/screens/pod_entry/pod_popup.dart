          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(width: 2, color: lightColorScheme.outlineVariant)
              ),
              child: Signature(
              ...
              children: [
                TextButton(onPressed: () async {
                  if (signController.isNotEmpty) {
                    ...
                    File finalFile = File(path.join(externalStorageDirectory!.path, path.basename('${DateTime.now().toIso8601String()}.jpg')));
                    await finalFile.writeAsBytes(sign!);
                    debugPrint(finalFile.path);
                    if (!context.mounted) return;
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: const Text('Signature Saved'),
                            showCloseIcon: true,
                ...
                style: const ButtonStyle(padding: MaterialStatePropertyAll(EdgeInsets.symmetric(horizontal: 24, vertical: 8))),
