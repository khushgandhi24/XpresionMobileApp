              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        children: [
                          ShipmentDetails(
                            awb: value.query,
                            date: value.shipment.data.first.transactiondate,
                            status: value.shipment.data.first.transactionstatus,
                          ),
                      const SizedBox(height: 24,),
                      Align(
                        alignment: const AlignmentDirectional(-1, 0),
                    ...
                    : (!value.isLoading) ?
                    Text((value.valid.isNotEmpty) ? (value.valid) : "", textAlign: TextAlign.center,)
                    : Center(
                    child: SizedBox(
                    height: MediaQuery.sizeOf(context).height * 0.05, width: MediaQuery.sizeOf(context).width * 0.1, child: CircularProgressIndicator(strokeWidth: 4,value: null, color: Theme.of(context).colorScheme.inverseSurface,)),
                  )
                    ,
                  ],
                ),
